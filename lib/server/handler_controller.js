...
export default class HandlerController {
  ...
  add (cursor, options) {
    ...
    // Building an observe handle yields, so a teardown can land in the middle of
    // it. The this.cursor path is covered by set()'s latch, this one has no such
    // step - and stop() has already emptied this.handlers, so storing it here
    // would leave it running against Mongo with nothing left to stop it.
    if (this._stopped) {
      if (handler && typeof handler.stop === 'function') handler.stop();
      return handler;
    }
