...
export default class HandlerController {
  ...
  add (cursor, options) {
    ...
    // FIX: previous handler for the same collection must be stopped before
    // we overwrite the reference, otherwise its observeChanges keeps running
    // with the older (smaller) $in selector and the leaked observer causes
    // "The Mongo server and the Meteor query disagree on how many documents
    // match your query" errors during oplog reconciliation.
    const oldHandler = this.handlers[collection];
