...
class CursorJoin {
  ...
  send () {
    ...
    // Everything this join publishes is pushed from callbacks of cursors
    // registered in the same body, and on Meteor 3 those only run once their
    // registration has finished. So wait for what the frame has already started:
    // otherwise send() reconciles an empty membership, releases its slot, and
    // the first real materialisation is left to a deferred restart nobody waits
    // for - which is how the publication would go ready before the joined
    // documents. This is the order a Fiber gave for free when the registrations
    // were synchronous.
    //
    // _settleStarted, not _settle: the promise below joins the same frame, and
    // the looping form would then wait for itself.
    const started = this.methods._settleStarted();

    return this.methods._track(started.then(() => this._reconcile()));
  }
  _scheduleCursor () {
    // Coalesce a burst of push()/drop calls (e.g. several parent docs
  ...
  //
  // _reconcile() handles its own failures and retries them, so the catch here is
  // for what is left over - a throw from the retry bookkeeping itself. Without
  // it that would surface as an unhandled rejection from a deferred callback,
  // with no publication left to attribute it to.
  _deferRestart (schedule, delay) {
    // Returned, though Meteor.defer ignores it: it is what lets a caller that
    // does drive the queue - the unit harness - wait for the restart to finish
    // rather than merely to have started.
    schedule(() => {
      const reconciling = runInContributor(null, () => this._reconcile());

      if (!isThenable(reconciling)) return reconciling;

      return reconciling.catch(error =>
        Meteor._debug('publish-relations: join on ' + this._name() + ' failed to restart', error));
    }, delay);
  }
  // The single place that turns declared contributions into published state.
  async _reconcile () {
    // _cursor() suspends, and the slot is released here so that a push arriving
    // while it is suspended still schedules a reconcile rather than being lost.
    // The cost is that the later run can finish first, leaving this one holding
    // a handle that set()'s latch has already thrown away.
    ...
    try {
      ...
      // Nothing left to observe. Not the same as observing {$in: []}, which is
      // what building the cursor anyway would do: that query can never match,
      // yet it still holds an ObserveMultiplexer and, under the oplog driver, a
      // trigger every write to the collection is tested against. So release the
      // slot instead - a later push builds a fresh one. This is also the state a
      // join that collected nothing is in when send() runs.
      if (!next.length) {
        // Defensive, not currently reachable: the only thing between the
        // generation stamp and here is _retract, and sub.removed makes no round
        // trip, so no other reconcile can have run. It guards the remove() below
        // rather than this.data - a superseded run must never stop an observer a
        // newer one owns - and costs one comparison to stay right if _retract
        // ever grows a yield.
        if (generation !== this._reconcileGeneration) return;

        // Not a failed state, just an empty one - a retry budget spent getting
        // here must not count against the next real membership.
        this._restartAttempt = 0;
        this.methods.handler.remove(this._registryKey);
        return;
      }

      const handle = await this._cursor();

      // Superseded while _cursor() was suspended: a later reconcile already owns
      ...
      // send() hands this back, so a caller can stop the join's observer. Only
      // on this path: a join whose membership is empty has no observer to stop
      // and send() returns nothing at all.
      return handle;
    } catch (error) {
