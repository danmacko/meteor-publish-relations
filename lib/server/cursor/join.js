...
class CursorJoin {
  ...
  _selector () {
    // Snapshot the ids: each observe must get its OWN frozen copy of the $in.
    // this.data is mutated in place (push/splice) and the observe restart is
    // deferred, so a live observe would otherwise have its compiled minimongo
    // matcher (frozen at the array contents when the matcher was built) drift
    // from the Mongo query (which re-reads the live array on every poll). That
    // mismatch is exactly what makes the oplog driver log "The Mongo server and
    // the Meteor query disagree on how many documents match your query".
